setwd('c:/seqgen/edgesandleaves')

source("topomatch.R")

treeNum <- 6

edgenum <- c("01","02","04","06","08","10")

bayesfiles <- lapply(1:6,function(i){paste("c:/seqgen/edgesandleaves/edge_",edgenum[i],"_bayes_toposummary.txt",sep="")})
raxmlfiles <- lapply(1:6,function(i){paste("c:/seqgen/edgesandleaves/edge_",edgenum[i],"_raxml_toposummary.txt",sep="")})
beastfiles <- lapply(1:6,function(i){paste("c:/seqgen/edgesandleaves/edge_",edgenum[i],"_BEAST_toposummary.txt",sep="")})
phymlfiles <- lapply(1:6,function(i){paste("c:/seqgen/edgesandleaves/edge_",edgenum[i],"_phyml_toposummary.txt",sep="")})
...
firstTopoSame <- lapply(1:6,function(i){checkTopo(1,i,1,1)})
# check if Topo #1 is the same as the second topo in the others
secondTopoSame <- lapply(1:6,function(i){checkTopo(1,i,2,1)})
...
bayesTopoMatched <- data.frame(x=rep(NA,6),y=rep(NA,2))
colnames(bayesTopoMatched) <- c(y1[1,1],y1[2,1])
bayesfirstTopoSame <- lapply(1:6,function(i){checkTopo("bayes",topoPick,i,1,1)})
bayessecondTopoSame <- lapply(1:6,function(i){checkTopo("bayes",topoPick,i,2,1)})
...
raxmlTopoMatched <- data.frame(x=rep(NA,6),y=rep(NA,2))
colnames(raxmlTopoMatched) <- c(y2[1,1],y2[2,1])
raxmlfirstTopoSame <- lapply(1:6,function(i){checkTopo("raxml",topoPick,i,1,1)})
raxmlsecondTopoSame <- lapply(1:6,function(i){checkTopo("raxml",topoPick,i,2,1)})

for (i in 1:6) {
  raxmlTopoMatched[i,1] <- raxmlfirstTopoSame[i]
  raxmlTopoMatched[i,2] <- raxmlsecondTopoSame[i]
}
beastTopoMatched <- data.frame(x=rep(NA,6),y=rep(NA,2))
colnames(beastTopoMatched) <- c(y3[1,1],y3[2,1])
beastfirstTopoSame <- lapply(1:6,function(i){checkTopo("beast",topoPick,i,1,1)})
beastsecondTopoSame <- lapply(1:6,function(i){checkTopo("beast",topoPick,i,2,1)})
...
phymlTopoMatched <- data.frame(x=rep(NA,6),y=rep(NA,2))
colnames(phymlTopoMatched) <- c(y4[1,1],y4[2,1])
phymlfirstTopoSame <- lapply(1:6,function(i){checkTopo("phyml",topoPick,i,1,1)})
phymlsecondTopoSame <- lapply(1:6,function(i){checkTopo("phyml",topoPick,i,2,1)})
...
bayesMatchOut <- paste("c:/seqgen/edgesandleaves/edge_bayes_",topoPick,"_topomatches.txt",sep="")
raxmlMatchOut <- paste("c:/seqgen/edgesandleaves/edge_raxml_",topoPick,"_topomatches.txt",sep="")
beastMatchOut <- paste("c:/seqgen/edgesandleaves/edge_beast_",topoPick,"_topomatches.txt",sep="")
phymlMatchOut <- paste("c:/seqgen/edgesandleaves/edge_phyml_",topoPick,"_topomatches.txt",sep="")
