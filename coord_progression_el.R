...
bayesCoords <- lapply(1:6, function(i){paste("c:/seqgen/edge",edgenum[i],"CDEgtr1/bayes/edge",edgenum[i],"CDEgtr1_bayes_allcoords.txt",sep="")})
raxmlCoords <- lapply(1:6, function(i){paste("c:/seqgen/edge",edgenum[i],"CDEgtr1/raxml/edge",edgenum[i],"CDEgtr1_raxml_allcoords.txt",sep="")})
beastCoords <- lapply(1:6, function(i){paste("c:/seqgen/edge",edgenum[i],"CDEgtr1/beast/edge",edgenum[i],"CDEgtr1_beast_allcoords.txt",sep="")})
phymlCoords <- lapply(1:6, function(i){paste("c:/seqgen/edge",edgenum[i],"CDEgtr1/phyml/edge",edgenum[i],"CDEgtr1_phyml_allcoords.txt",sep="")})
...
plot.progression <- function(model,targetdir){
  setwd(targetdir)
  pngfile = paste(targetdir,"edge_topo_prog_",model,".png")
...
plot.comparison <- function(targetdir){
  setwd(targetdir)
  pngfile = paste(targetdir,"edge_topo_comp.png")
  png(pngfile,width=1000,height=1000)
...
{
  setwd(targetdir)
  pngfile = paste(targetdir,"edge_topo_comp_progcolors.png")
  png(pngfile,width=1000,height=1000)
  models <- c("bayes","raxml","beast","phyml")
  #models <- c("bayes","raxml","phyml")
  for (j in 1:length(models)){
