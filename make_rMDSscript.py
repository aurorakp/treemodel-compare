...
class make_rMDSscript(object):
    ...
    def plot_MDS(self,k=2):
        ...
        if (k == 2):
            outfile.write('plot(x,y,col = plotcolors.palette(1003),pch=c(plotpch),main="'+ self.title + self.asp+')\n')
        elif (k == 3):
            outfile.write('scatter3D(x,y,z,col = plotcolors.palette(1003),pch=c(plotpch),main="'+ self.title + self.asp+')\n')
    ...
    def run_MDSplots(self,k=2):
        rfile = self.filepath + self.treename + self.suffix + str(k) + "D.r"
        command = 'C:\\Rstuff\\R-3.2.0\\bin\\Rscript.exe ' + rfile
        call(command.split())
